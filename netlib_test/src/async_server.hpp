...
struct file_cache 
{
...
#else
		int fd = _open(real_filename.c_str(), O_RDONLY);
#endif
		if (fd == -1) return false;

		std::size_t size = _lseek(fd, 0, SEEK_END);

		file_name_ss = real_filename;
		file_size = size;

		void * region = mmap(0, size, PROT_READ, MAP_PRIVATE, fd, 0);
		if (region == MAP_FAILED) { _close(fd); return false; }
	...
	boost::iterator_range<std::vector<server::response_header>::iterator> meta(std::string const & path) 
	{
		...
		{
			std::vector<server::response_header>::iterator begin = headers->second.begin(), end = headers->second.end();
...
struct connection_handler : boost::enable_shared_from_this<connection_handler> 
{
	...
	void operator()(std::string const & path, server::connection_ptr connection, bool serve_body) 
	{
		...
		if (true) 
		{
			...
			if (serve_body) 
				send_file_2( file_cache_.file_name_ss, 0, file_cache_.file_size, connection );
	...
	void send_file_2( std::string filename, int begin, std::size_t file_size, server::connection_ptr connection )
	{
		...
		if( infile.gcount() <= 0 )
			return;
		connection->write(  boost::asio::const_buffers_1( static_cast<char const *>(buf), 4096  )  ,
				boost::bind( &connection_handler::handle_chunk_2, 
					connection_handler::shared_from_this(),
					filename, begin+4096, file_size, connection, _1 ) 
			);
	}

	void handle_chunk_2( std::string filename, int begin, std::size_t file_size, server::connection_ptr connection, boost::system::error_code const & ec) 
	{
		//if (!ec && offset < mmaped_region.second) 
		send_file_2(filename, begin, file_size, connection);
	}
